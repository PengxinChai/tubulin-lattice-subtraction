...
int main(int argc, char *argv[])
{
  ...
  if (argc < 5)
  {
    fprintf(stderr, "\nUSAGE: %s <mrc> <mask> <coords_file> <scale_factor>\n", argv[0]);
    fprintf(stderr, "\nOnly support float16 mrc file format (for both image and mask!!) and mrc file must end with .mrc.\n");
    ...
    fprintf(stderr, "The width of the mask should be a litter bigger than the width of filament. See the example masks we provided.\n");
    fprintf(stderr, "Scale factor 0 means subtract all signals of repeating unit. 1 means no subtractoin. Usually set to 0\n");

    fprintf(stderr, "\nExample: %s Slot11_3_100_00004_F50_MC2_DW_shrink2.mrc MT_mask_400_32.mrc Slot11_3_100_00004_F50_MC2_DW_shrink2.txt 0\n", argv[0]);
  ...
  while(end < coords->size){
    if (coords->elements[end].cluster != coords->elements[start].cluster){
      ...
      subtract_repeat_signal(mrc_image,mask, template,X,Y,A, start, end-1, sigma);
  ...
  printf("Wrote: %s\n\n",mrc_output);
  write_mrc(output_path,mrc_image); 
  //free structs 
  free_mrc(mask);
  free_mrc(template);
  free_mrc(mrc_image);
