...
float float16_to_float32(uint16_t h) {
    ...
    uint16_t h_sgn = (h & 0x8000) >> 15;

    uint32_t f_sgn = h_sgn << 31;
...
uint16_t float32_to_float16(float f_val) {
    uint32_t f = *((uint32_t*)&f_val);
    uint16_t h_sgn = (f >> 16) & 0x8000;
    uint32_t f_exp = (f >> 23) & 0xFF;
    uint32_t f_sig = f & 0x7FFFFF;
    ...
    if (f_exp < 103) {
        ...
        // Subnormal
        uint32_t shift = 113 - f_exp;
        h_sig = (0x800000 | f_sig) >> (shift + 13);
        h_exp = 0;
    } else {
        h_exp = f_exp - 112;
        h_sig = f_sig >> 13;
...
void print_mrc_header(mrc m){
  ...
  switch (hdr->dtype)
  {
    ...
    case 6:
      ...
      break;
    case 12:
      printf("16-bit signed real, float16\n");
      break;
...
mrc read_mrc(FILE *in){
  ...
  {
    case 12:
      elt_size = sizeof(float16_raw);
      tmp = malloc(sizeof(float16_raw **)*num_row);
      for (int32_t r=0; r<num_row; r++){
        tmp[r] = malloc(sizeof(float16_raw *)*num_col);
        for(int32_t c=0; c<num_col;c++){
          tmp[r][c] = malloc(sizeof(float16_raw)*num_sec);
        }
      }
      break;
    default:
      printf("ERROR: Only support float16 data type. Please convert both the image and the mask to float16.\n");
      return NULL;
      exit(0);
  }
  ...
  for(int32_t s=0; s<num_sec;s++){
    for (int32_t r=0; r<num_row; r++){
      for(int32_t c=0; c<num_col;c++){
	float16_raw read_f16 = read_float16(in);
        // fread((char *)tmp[s][r]+c*elt_size, 2, 1 , in);
	
	float my_float32 = float16_to_float32(read_f16);
        
	m->data[s][r][c] = my_float32;
...
void write_mrc(FILE *out, mrc m){
  ...
  for(int32_t s=0; s<num_sec;s++){
    for (int32_t r=0; r<num_row; r++){
      for(int32_t c=0; c<num_col;c++){
	float my_float32= data[s][r][c];
	float16_raw write_f16=float32_to_float16(my_float32);
        //fwrite(data[s][r]+c,4,1,out);
	write_float16(out,write_f16);
      }
    }
  }  
}
